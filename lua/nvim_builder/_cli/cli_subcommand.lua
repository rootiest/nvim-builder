...
---
---@module 'nvim_builder._cli.cli_subcommand'
---
...
---@class nvim_builder.NamespaceExecuteArguments
---    The expected data that's passed to any `set_execute` call in nvim-builder.
...
---@class nvim_builder.CompleteData
---    The data that gets passed when `nvim_builder.Subcommand.complete` is called.
---@field input argparse.Results
---    All information that was found from parsing some user's input.

---@class nvim_builder.RunData
---    The data that gets passed when `nvim_builder.Subcommand.run` is called.
---@field input argparse.Results
---    All information that was found from parsing some user's input.

---@class nvim_builder.Subcommand
...
---@field complete (fun(data: nvim_builder.CompleteData): string[])?
...
---@field run (fun(data: nvim_builder.SubcommandRun): nil)?
---    The function to run when the subcommand is called.

---@class nvim_builder.SubcommandRun
...
---@alias nvim_builder.ParserCreator fun(): cmdparse.ParameterParser

---@alias nvim_builder.Subcommands table<string, nvim_builder.Subcommand | fun(): cmdparse.ParameterParser>

--- Check if `full` contains `prefix` + whitespace.
---
---@param full string Some full text like `"NvimBuilder blah"`.
---@param prefix string The expected starting text. e.g. `"NvimBuilder"`.
...
---
---@param text string Some full text like `"NvimBuilder blah"`.
---@param prefix string The expected starting text. e.g. `"NvimBuilder"`.
---@param subcommands nvim_builder.Subcommands All allowed commands.
---
local function _get_subcommand_completion(text, prefix, subcommands)
    local argparse = require("nvim_builder._cli.argparse")
    ...
    if not subcommands[subcommand_key] then
        vim.notify(
            string.format(
                'NvimBuilder: Unknown command "%s". Please check your spelling and try again.',
                subcommand_key
            ),
    ...
    ---@cast subcommand nvim_builder.Subcommand
    ...
    if subcommand.complete then
        ...
        if result == nil or vim.islist(result) then
            ...
            vim.notify(
                string.format(
                    'nvim-builder: Subcommand / Arguments "%s / %s" must be a string[]. Got "%s".',
                    subcommand,
                    arguments,
                    vim.inspect(result)
                )
...
local function _run_subcommand(parser, text)
    ...
    ---@type fun(data: nvim_builder.NamespaceExecuteArguments): nil
    ...
    vim.notify(
        string.format(
            'NvimBuilder: Command "%s" parsed "%s" text into "%s" namespace but no `execute` '
...
---
---@param subcommands nvim_builder.Subcommands
---    All registered commands for `nvim_builder` to possibly modify.
---
...
---
---@param parser_creator nvim_builder.ParserCreator
...
---
---@param prefix string The command to exclude from auto-complete. e.g. `"NvimBuilder"`.
---@param subcommands nvim_builder.Subcommands All allowed commands.
...
function M.make_subcommand_triager(subcommands)
    ...
    local function _runner(opts)
        ...
        if not subcommand then
            vim.notify("NvimBuilder: Unknown command: " .. subcommand_key, vim.log.levels.ERROR)
    ...
    local function runner(opts)
        ...
        local help_message = require("nvim_builder._cli.cmdparse.help_message")
