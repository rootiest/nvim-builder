--- Make sure `nvim_builder` will work as expected.
---
--- At minimum, we validate that the user's configuration is correct. But other
--- checks can happen here if needed.
---
---@module 'nvim_builder.health'
---

local configuration_ = require("nvim_builder._core.configuration")
...
---
---@param data nvim_builder.Configuration All of the user's fallback settings.
...
---
---@param command string A supported `nvim_builder` command. e.g. `"hello_world"`.
---@return string[] # All found issues, if any.
---
...
---
---@param data nvim_builder.LoggingConfiguration The user's logger settings.
...
function M.get_issues(data)
    if not data or vim.tbl_isempty(data) then
        data = configuration_.resolve_data(vim.g.nvim_builder_configuration)
    end
...
--- Make sure `data` will work for `nvim_builder`.
---
---@param data nvim_builder.Configuration? All extra customizations for this plugin.
---
function M.check(data)
    vlog.debug("Running nvim-builder health check.")
    ...
    if vim.tbl_isempty(issues) then
        vim.health.ok("Your vim.g.nvim_builder_configuration variable is great!")
    end
